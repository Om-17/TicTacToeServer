...
class DBClass extends DBconnection
{
  public $table;
  ...
  public function aexists($conditions)
  {
    ...
    try {
      ...
      return ($result != 0) ? 1 : 0; // Record exists or not
    } catch (\Throwable $th) {
      // Handle the error as per your requirements
      return 1;
    }
  ...
  public function adelete($delete_conditions)
  {
    $context = [];
    ...
    if (!empty($delete_conditions)) {
      ...
      $whereConditions = [];
      foreach ($delete_conditions as $field => $fieldValue) {
        $whereConditions[] = "$field = :$field";
      }
